...
if($requestData['action'] === 'deleteStudent') {
    $data = $requestData['data'];
...
if($requestData['action'] === 'deleteEmployee'){
    $data = $requestData['data'];
    $medicareID = $data['medicareID'];
    //Response message
    $message = "medicareID: $medicareID\nData has been processed.";
    //Delete data from the database
    $sql = "DELETE FROM Employees WHERE medicareID = $medicareID;";
    if ($conn->query($sql) === TRUE) {
        $response = array('message' => 'Data inserted successfully');
    } else {
        $response = array('message' => 'Error inserting data: ' . $conn->error);
...
if($requestData['action'] === 'deleteSchedule'){
try{
    $data = $requestData['data'];
    $medicareID = $data['medicareID'];
    $date = $data['date'];
...
if($requestData['action'] === 'deleteInfection'){
    try{
    $data = $requestData['data'];
    $medicareID = $data['medicareID'];
...
if($requestData['action'] === 'deleteVaccination'){
    try{
    $data = $requestData['data'];
    $medicareID = $data['medicareID'];
